...
uart = serial.Serial('com29', 115200)  # 在 windows 下控制电机测试，相应的输入连接的COM口和波特率
...
def init_data():
    global uart
    init_msg = [0x41, 0x54, 0x2B, 0x41, 0x54, 0x0D, 0x0A]
    uart.write(init_msg)
...
def can_to_uart(data=[], rtr=0):
    udata = [0x41, 0x54, 0, 0, 0, 0, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0D, 0x0A]
    # yudata = [0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]


    if len(data) == 13:
        '''
        /*fdata = data[1] * 16* 6 + data[2] * 16*4 + data[3] * 16* 2 + data[4]
        f2data = (fdata << 3) + 0x04
        udata[2] = f2data >> 24 & 0xFF
        udata[3] = f2data >> 16 & 0xFF
        udata[4] = f2data >> 8 & 0xFF
        udata[5] = f2data8*///
        '''
        udata[2] = (data[1] << 3) & 0xf8 | (data[2] >> 5) & 0x07
        udata[3] = (data[2] << 3) & 0xf8 | (data[3] >> 5) & 0x07
        udata[4] = (data[3] << 3) & 0xf8 | (data[4] >> 5) & 0x07
        udata[5] = (data[4] << 3) & 0xf8 | 0x04
        udata[7] = data[5]
        udata[8] = data[6]
        udata[9] = data[7]
        udata[10] = data[8]
...
def uart_to_can(data=[]):
    ...
    if len(data) == 17 and data[0] == 0x41 and data[1] == 0x54:
        for i in range(12):
            cdata[1 + i] = data[i + 4]
        return cdata
    else:
...
# CAN发送函数
def send_command(id_num=127, cmd_mode=0, cmd_data=[], data=[], rtr=0):
    cdata = [0x08, 0, 0, 0, 0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]
    cdata[1] = cmd_mode
    cdata[2] = cmd_data[1]
...
# 速度控制
def set_speed(id_num=127, speed=10, limit_cur=23):
    """电机速度控制函数。
